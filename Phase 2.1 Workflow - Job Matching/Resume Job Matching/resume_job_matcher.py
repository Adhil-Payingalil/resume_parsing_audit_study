...
This module provides the core functionality for matching resumes to job postings
using a two-stage approach: vector search followed by LLM validation.
"""
...
class ResumeJobMatcher:
    """
    Main class for matching resumes to job postings using vector search and LLM validation.
    """
    
    def __init__(self, db_name: str = "Resume_study"):
        """
        Initialize the ResumeJobMatcher.
        ...
        logger.info(f"ResumeJobMatcher initialized for database: {db_name}")
    ...
    def vector_search_resumes(self, job_doc: Dict[str, Any], top_k: int = 4) -> List[Dict[str, Any]]:
        """
        Perform vector search to find the most similar resumes for a job.
        ...
        try:
            ...
            logger.info(f"Found {len(top_resumes)} top resumes for job {job_doc.get('_id')}")
            return top_resumes
...
"""
    ...
    def store_unmatched_job(self, job_doc: Dict[str, Any], top_similarity_score: float):
        """
        ...
        try:
            ...
            logger.info(f"Stored unmatched job {job_doc.get('_id')}")
    ...
    def process_job_matching(self, job_doc: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process matching for a single job posting.
        ...
        """
        try:
            logger.info(f"Processing job matching for job {job_doc.get('_id')}: {job_doc.get('job_title')}")
            
            # Stage 1: Vector search for top resumes
            top_resumes = self.vector_search_resumes(job_doc, top_k=4)
            ...
            if not validated_matches:
                self.store_unmatched_job(job_doc, top_similarity_score)
                return {
            ...
            logger.info(f"Created {len(validated_matches)} valid matches for job {job_doc.get('_id')}")
            return {
                "status": "success",
            ...
            return {"status": "error", "error": str(e), "matches_created": 0}
